...
const Gallery = ({ position }) => {
  ...
  console.log(window.innerWidth / 2 - position.x);
    ...
    <Box>
      ...
      <Box
        sx={{
          ...
          width: "60%",
          bottom: "2.5%",
          transform: `translate(calc(35% - ${position.x / 50}px), calc(-${position.y / 20}px)) 
          rotateX(${(window.innerHeight / 2 - position.y) / 20}deg) 
          rotateY(${-(window.innerHeight / 2 - position.x) / 20}deg)`,
          perspective:"1000px",
          transition: "0s",
          transitionTimingFunction: "ease-out",
        }}
